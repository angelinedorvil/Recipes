...
class Recipe:
  ...
  def save2json(self, filename):
    recipeDict = {'name': self.name, 'ingredients': self.ingredients, 'instructions': self.instructions, 'category': self.category, 'calories': self.calories, 'carbs': self.carbs, 'protein': self.protein}
...
def updateRecipe(recipe_name):
    ...
    while True:
        ...
        if userInput == '1':
          ...
          # Updating category
          new_category = get_valid_input("Enter the new category of the recipe: ", lambda x: len(x) > 0 and not x.isdigit())
...
def update_ingredients(recipe_name):
    ...
    while True:
      ...
      if ingredient_action == '1':
        new_ingredients_input = get_valid_input("Enter the new ingredients separated by commas: ", lambda x: len(x) > 0)
        ...
        ingredient_to_remove = get_valid_input("Enter the ingredient to remove: ", lambda x: len(x) > 0 and not x.isdigit())
        if ingredient_to_remove in current_ingredients:
          recipes[recipe_name].ingredients = [ing for ing in recipes[recipe_name].ingredients if ing != ingredient_to_remove]
          print("")
        ...
        new_ingredients = get_valid_input("Enter the new ingredients separated by commas: ", lambda x: len(x) > 0 and not x.isdigit())
...
def update_instructions(recipe_name):
  ...
  while True:
    ...
    if instruction_action == '1':
      new_instructions_input = get_valid_input("Enter the new instructions separated by periods: ", lambda x: len(x) > 0)
      ...
      new_instructions = get_valid_input("Enter the new instructions separated by periods: ", lambda x: len(x) > 0)
...
def addRecipe():
  ...
  while True:
    ingredients = [ing.strip() for ing in input("Enter the ingredients of the recipe (comma-separated): ").split(',')]
  ...
  while True:
    instructions = [ins.strip() for ins in input("Enter the  instructions of the recipe (period-separated): ").split('.')]
  ...
  newRecipe = Recipe(name, ingredients, instructions, category, calories, carbs, protein)
...
def main():
  ...
  initial_recipes_file = 'initial_recipes.json'  # Replace with your JSON file name
  loadAllFromjson(initial_recipes_file)
  auto_save()
